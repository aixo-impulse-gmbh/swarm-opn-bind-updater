...
import opnsense_bind_companion
...
def test_load_configuration():
    config = opnsense_bind_companion.load_configuration()

    assert config != None
    assert config["GW_BASE_URL"] == "https://gateway.c.internal"
    assert config["GW_API_KEY"] != None
    assert config["GW_API_SECRET"] != None
...
def test_can_create_record_payload():
    ...
    actual = opnsense_bind_companion._create_record_payload(domain_id, name, type, value)
...
def test_find_domain_id(requests_mock : Mocker):
    ...
    domain_id = opnsense_bind_companion.find_domain(API_KEY, API_SECRET, BASE_URL, domain_name)
...
def test_add_host(requests_mock : Mocker):
    ...
    opnsense_bind_companion.add_record(API_KEY, API_SECRET, BASE_URL, "dbf3748c-ac5a-4512-8941-4b8c10d3558d", "blog")

def test_add_host_failed(requests_mock : Mocker):
    requests_mock.post(url=BIND_RECORD_ADDRECORD, status_code=400, text="dah")

    with pytest.raises(Exception) as ex:
        opnsense_bind_companion.add_record(API_KEY, API_SECRET, BASE_URL, "b78013ea-cdaf-4335-b429-7af99b2f9a12", "blog")
...
def test_remove_host(requests_mock : Mocker):
    ...
    opnsense_bind_companion.remove_record(API_KEY, API_SECRET, BASE_URL, record_id)
